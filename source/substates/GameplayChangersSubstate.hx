...
import options.BaseOptionsMenu.Checkbutton;
...
class GameplayChangersSubstate extends MusicBeatSubstate
{
	...
	private var grpTexts:FlxTypedGroup<MenuText>;
	...
	public function new()
	{
		...
		grpTexts = new FlxTypedGroup<MenuText>();
		add(grpTexts);
		...
		{
			var optionText:MenuText = new MenuText(200, 360, 0, optionsArray[i].name, 32);
			...
			if(optionsArray[i].type == 'bool') {
				...
				var checkbox:Checkbutton = new Checkbutton(optionText.x - 105, optionText.y, optionsArray[i].getValue() == true);
				...
				optionText.snapToPosition();
				var valueText:MenuText = new MenuText(optionText.width + 40, 0, 0, Std.string(optionsArray[i].getValue()), 32);
